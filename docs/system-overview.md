...
- 来源健康已展示权重、SLA、失败、待配置、失败率、重试次数和平均耗时。
- 榜单评分已按模型、Agent、工具三套规则拆分 `scoreBreakdown`，后续仍需用历史样本继续校准各权重阈值。
- 线上 GitHub Actions 已触发，但当前 `main` 最近运行失败在构建依赖缺失；需合入当前依赖和锁文件修复后复查自动提交。
