...
function limitRankingItems(baseItems: RankingItem[], target: number) {
  return baseItems.slice(0, target)
}

function compactSourceRows(sources: SourceDefinition[], target = 6) {
  return sources.slice(0, target).map((source) => ({
    id: source.id,
    name: source.name,
      ...
      <tbody>
        {items.slice(0, 8).map((item, index) => (
          <tr key={`${item.kind}-${item.name}`}>
            ...
            <td>待接入</td>
            <td>待接入</td>
            <td><StatusBadge tone={scoreTone(item.score)}>{rankingScoreLabel(item.score)}</StatusBadge></td>
  ...
  return (
    <aside className="fi-strict-panel fi-strict-side-panel">
      ...
      <div className="fi-strict-progress-list">
        {visible.map((source) => {
          const coverage = Math.round(Math.max(0, Math.min(100, source.weight * 100)))
          return (
          <div key={source.id}>
            <span>{sourceTypeLabel(source.sourceType)}<strong>{coverage}%</strong></span>
            <div><i style={{ width: `${coverage}%` }} /></div>
          </div>
          )
        })}
        {!visible.length && <p className="fi-strict-warning-note">当前筛选下暂无来源覆盖数据。</p>}
      </div>
      <div className="fi-strict-source-tile-grid">
        {visible.map((source) => (
          <article key={source.id}>
            <strong>{source.name}</strong>
            <small>{source.status} · 权重 {Math.round(source.weight * 100)}%</small>
          </article>
        ))}
      </div>
    </aside>
...
export function StrictDomainPage({ abilityTags, dataset, items, kind, relatedSignals, sources }: StrictDomainPageProps) {
  ...
  const modelItems = limitRankingItems(items.length ? items : dataset.rankingItems.filter((item) => item.kind === 'model'), 8)
  ...
  const modelSourceCount = sources.length
  const modelRankingCount = dataset.rankingItems.filter((item) => item.kind === 'model').length
  const bestConfidence = Math.max(...relatedSignals.map((signal) => signal.confidence), 0)

  return (
    <section className="fi-strict-page">
      <div className="fi-strict-kpi-grid">
        <StrictKpi icon={<Box size={22} />} label="模型条目" note="来自 rankingItems.kind=model" tone="blue" value={modelItems.length} />
        <StrictKpi icon={<Trophy size={22} />} label="榜单条目" note="当前数据快照" tone="orange" value={modelRankingCount} />
        <StrictKpi icon={<Star size={22} />} label="最高置信信号" note={relatedSignals[0]?.title ?? '当前筛选暂无信号'} tone="purple" value={bestConfidence || '暂无'} />
        <StrictKpi icon={<Clock3 size={22} />} label="模型来源" note={`刷新于 ${formatDate(dataset.generatedAt)}`} tone="teal" value={modelSourceCount} />
      </div>
      <div className="fi-strict-two-col">
        <section className="fi-strict-panel fi-strict-table-card">
          ...
          <TableFooter count={modelItems.length} />
        </section>
    ...
    <div className="fi-strict-agent-grid">
      {items.slice(0, 10).map((item) => (
        <article className={selectedName === item.name ? 'is-selected' : ''} key={item.name} onClick={() => onSelect(item.name)} onKeyDown={(event) => {
          ...
          <b>{rankingScoreLabel(item.score)}</b>
          <StatusBadge tone={scoreTone(item.score)}>{item.rank ? `#${item.rank}` : '观察'}</StatusBadge>
          <p>来源：{item.source ?? item.provider}</p>
          <div><span>{item.category}</span><span>{item.kind ?? 'agent'}</span><span>{item.source ? '有来源' : '待来源'}</span></div>
        </article>
      ))}
    </div>
  )
...
function StrictAgentPage({ items, sources }: { items: RankingItem[]; relatedSignals: FrontierSignal[]; sources: SourceDefinition[] }) {
  const [selectedName, setSelectedName] = useState('')
  const [agentFilter, setAgentFilter] = useState('热门')
  const [detailOpen, setDetailOpen] = useState(true)
  const allItems = limitRankingItems(items, 10)
  ...
  const displayItems = filteredAgentItems
      ...
      <div className="fi-strict-two-col is-agent">
        <div className="fi-strict-left">
          ...
          <section className="fi-strict-panel fi-strict-table-card">
            ...
            <TableFooter count={displayItems.length} />
          </section>
        </div>
        <aside className="fi-strict-side-stack">
          <section className="fi-strict-panel">
            ...
            <div className="fi-strict-category-grid">
              {sources.slice(0, 8).map((source) => (
                <article key={source.id}><StatusBadge tone={source.status === 'ok' ? 'teal' : 'blue'}>{sourceTypeLabel(source.sourceType)}</StatusBadge><strong>{Math.round(source.weight * 100)}</strong></article>
              ))}
              {!sources.length && <article><StatusBadge tone="warning">待接入</StatusBadge><strong>0</strong></article>}
            </div>
          </section>
          ...
          {detailOpen ? <section className="fi-strict-panel">
            ...
            <h4>{selectedItem?.name ?? '暂无 Agent'}</h4>
            <p>{selectedItem?.scoringExplanation ?? selectedItem?.source ?? '当前筛选下暂无可展示的 Agent 明细。'}</p>
            <div className="fi-strict-source-tile-grid">
...
function StrictSkillPage({ items, relatedSignals, sources }: { items: RankingItem[]; relatedSignals: FrontierSignal[]; sources: SourceDefinition[] }) {
  const [category, setCategory] = useState('全部')
  const [selectedName, setSelectedName] = useState('')
  const rows = limitRankingItems(items, 9)
  ...
  const selectedRow = visibleRows.find((item) => item.name === selectedName) ?? visibleRows[0]
  const toolCategoryCounts = Object.entries(
    ...
    <section className="fi-strict-page">
      ...
      <div className="fi-strict-two-col">
        <section className="fi-strict-panel fi-strict-table-card">
            ...
            <tbody>
              {visibleRows.slice(0, 9).map((item, index) => (
                <tr className={selectedRow?.name === item.name ? 'is-selected' : ''} key={item.name} onClick={() => setSelectedName(item.name)} role="button" tabIndex={0}>
                  ...
                  <td>{typeof item.change === 'number' ? `${item.change > 0 ? '↑ ' : ''}${item.change}` : '待历史数据'}</td>
                  <td>{rankingScoreLabel(item.score)}</td>
          ...
          <TableFooter count={visibleRows.length} />
        </section>
        <aside className="fi-strict-side-stack">
          <DistributionPanel sources={sources} />
          <section className="fi-strict-panel">
            <div className="fi-strict-card-head"><h3>选中工具</h3></div>
            <h4>{selectedRow?.name ?? '暂无工具'}</h4>
            <p>{selectedRow?.scoringExplanation ?? selectedRow?.source ?? '当前筛选下暂无可展示的工具明细。'}</p>
          </section>
          <section className="fi-strict-panel">
            <div className="fi-strict-card-head"><h3>能力分类</h3></div>
            <div className="fi-strict-category-grid">
              {toolCategoryCounts.map(([item, count], index) => (
                <article key={item}><StatusBadge tone={index % 3 === 0 ? 'purple' : 'blue'}>{item}</StatusBadge><strong>{count}</strong></article>
              ))}
              {!toolCategoryCounts.length && <article><StatusBadge tone="warning">待接入</StatusBadge><strong>0</strong></article>}
            </div>
...
function DistributionPanel({ sources }: { sources: SourceDefinition[] }) {
  ...
  const totalWeight = rows.reduce((sum, source) => sum + source.weight, 0)
  return (
    <section className="fi-strict-panel">
      <div className="fi-strict-card-head"><h3>来源分布</h3></div>
      <div className="fi-strict-progress-list is-purple">
        {rows.map((source) => {
          const percent = Math.round((source.weight / Math.max(totalWeight, 0.01)) * 100)
          return (
          <div key={source.id}>
            <span>{source.name}<strong>{percent}%</strong></span>
            <div><i style={{ width: `${percent}%` }} /></div>
          </div>
          )
        })}
        {!rows.length && <p className="fi-strict-warning-note">当前筛选下暂无来源分布数据。</p>}
      </div>
    </section>
  )
}
  ...
  return (
    <section className="fi-strict-panel">
      ...
      <table className="fi-strict-mini-table">
        <tbody>
          {rows.map((source) => {
            const health = source.status === 'ok' ? Math.round(source.weight * 100) : source.status === 'skipped' ? 0 : 25
            return (
              <tr key={source.id}>
                <td>{source.name}</td>
                <td><StatusBadge status={source.status}>{source.status}</StatusBadge></td>
                <td>{health}%</td>
                <td><div className="fi-strict-meter"><span style={{ width: `${health}%` }} /></div></td>
              </tr>
            )
          })}
          {!rows.length && <tr><td colSpan={4}>暂无来源健康数据</td></tr>}
        </tbody>
      </table>
    </section>
  )
...
export function StrictCalendarPage({ dataset }: { dataset: FrontierIntelDataset }) {
  const [viewMode, setViewMode] = useState<'窗口' | '列表'>('窗口')
  ...
  return (
    <section className="fi-strict-page">
      <StrictToolbar>
        {(['窗口', '列表'] as const).map((item) => <button aria-pressed={viewMode === item} className={viewMode === item ? 'is-active' : ''} key={item} onClick={() => setViewMode(item)} type="button">{item}</button>)}
        ...
        <section className="fi-strict-panel fi-strict-calendar-panel">
          {viewMode === '列表' ? (
            <div className="fi-strict-release-list">
              ...
              {!events.length && <EmptyStrictPanel title="当前筛选下暂无发布信号" />}
            </div>
          ) : <div className="fi-strict-release-list">
            {windowGroups.map(([window, rows]) => (
              <section key={window}>
                <div className="fi-strict-card-head"><h3>{window}</h3><span>{rows.length} 条</span></div>
                {rows.map((event) => (
            ...
            {!windowGroups.length && <EmptyStrictPanel title="当前筛选下暂无发布窗口" />}
            </div>
          }
...
export function StrictRoadmapPage({ items }: StrictRoadmapPageProps) {
  ...
  return (
    <section className="fi-strict-page">
      <StrictToolbar>
        {(['时间线', '里程碑'] as const).map((item) => <button aria-pressed={viewMode === item} className={viewMode === item ? 'is-active' : ''} key={item} onClick={() => setViewMode(item)} type="button">{item}</button>)}
        ...
        <SelectLike active={viewMode === '里程碑'} label="视图" onClick={() => setViewMode(viewMode === '里程碑' ? '时间线' : '里程碑')} value={viewMode} />
      ...
      <div className="fi-strict-roadmap-grid">
        ...
        <section className="fi-strict-panel fi-strict-gantt">
          <div className="fi-strict-card-head"><h3>路线图状态</h3><StatusBadge tone="blue">roadmapItems</StatusBadge></div>
          <div className="fi-strict-category-grid">
            {[
              ['已完成', completedCount, 'done'],
              ['进行中', activeCount, 'active'],
              ['计划中', plannedCount, 'planned'],
              ['总项目', roadmapCards.length, 'system'],
        ...
        <aside className="fi-strict-side-stack">
          ...
          <section className="fi-strict-panel">
            ...
            <table className="fi-strict-mini-table"><tbody>
              <tr><td>done</td><td>{completedCount}</td></tr>
              <tr><td>active</td><td>{activeCount}</td></tr>
              <tr><td>planned</td><td>{plannedCount}</td></tr>
            </tbody></table>
          </section>
          <section className="fi-strict-panel">
