...
async function runDomainInteractions(page, baseUrl, outDir) {
  ...
  await resetGlobalFilters(page)
  await clickText(page, '.fi-strict-toolbar button', 'Coding')
  await page.waitForTimeout(150)
  const secondAgent = page.locator('.fi-strict-agent-grid article[role="button"]').nth(1)
...
async function runCalendarSourcesRoadmapInteractions(page, baseUrl, outDir) {
  ...
  await page.locator('.fi-strict-toolbar button').filter({ hasText: '›' }).first().click()
  await page.waitForTimeout(150)
  await screenshotInteraction(page, outDir, '08-release-calendar-next-month.png', results, 'calendar-next-month')
  ...
  await resetGlobalFilters(page)
  await clickText(page, '.fi-strict-toolbar button', '甘特图')
  await page.waitForTimeout(150)
  await screenshotInteraction(page, outDir, '10-roadmap-gantt-view.png', results, 'roadmap-gantt-view')
